...
#define RUN_TEST(test) {\
  ...
  output_file << #test << "...";\
  try { test(); }\
  catch (std::exception& e) { mt_ok = false; output_file << "failed\n" << e.what(); } \
  catch (const char* msg) { mt_ok = false; output_file << "failed\n" << msg; } \
  catch (...) { mt_ok = false; output_file << "failed\n"; }\
  if (mt_ok) { output_file << "ok"; }\
  output_file << "\n";\
  }\
}
